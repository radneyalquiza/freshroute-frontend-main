    ...
    const sys = {
        ...
        makeDevServerChanges() {
            let defer = new Q.defer(),
                ...
                defaultCsp = `default-src *; script-src 'self' data: 'unsafe-inline' 'unsafe-eval' http://127.0.0.1:8001 http://LOCIP:8001; object-src 'self' data: http://127.0.0.1:8001 http://LOCIP:8001; style-src 'self' 'unsafe-inline' data: ; img-src *; media-src 'self' data: http://127.0.0.1:8001 http://LOCIP:8001; frame-src 'self' data: http://127.0.0.1:8001 http://LOCIP:8001; font-src *; connect-src 'self' data: http://127.0.0.1:8001 http://LOCIP:8001`,
        ...
        startWebpackDevServer() {
            let defer = new Q.defer(),
                outText = '',
                isResultFound = false,
                args = [`"${webpackDevServerPath}"`, '--hot', '--inline', '--env.devserver', `--public ${getRouterIpAddr()}:8001`],
                run = epipeBombPath

            if (os.platform() === 'win32') {
                args = ['--hot', '--inline', '--env.devserver', `--public ${getRouterIpAddr()}:8001`]
