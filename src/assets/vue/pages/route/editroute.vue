<template>
	<f7-page class="routelist">
		<f7-navbar title="Edit Route" back-link=" ">
		</f7-navbar>
...
export default {
		...
		attachedemployees: function(newval, oldval) {
			...
			if(newval && newval.length && instance.employees.length)
			{
				...
				for(var x=0; x<newval.length; x++) {
					...
					// })
					// if(s) instance.currentWorkers.push({ AppUser: s, Pay: 0 });
    ...
    mounted() {
		...
		for(var x in instance.route.Workers) {
			let emp = {
				AppUser: instance.route.Workers[x].AppUser,
				Pay: instance.route.Workers[x].Pay
			}

			instance.attachedemployees.push(emp.AppUser.AppUserId);
